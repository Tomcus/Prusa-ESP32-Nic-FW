...
void RxParserBase::process_data(std::span<const uint8_t> input) {
    ...
    while (curr != end) {
        ...
        if (state == State::Header) {
            if (wait_for_buffer()) {
                buffer = std::span { reinterpret_cast<uint8_t *>(&msg.header), sizeof(msg.header) };
                fl16.update(buffer);
...
bool RxParserBase::wait_for_data() {
    ...
    if (state == State::Data) {
        ...
        fl16.update(to_be_processed_data);
...
void RxParserBase::on_parsed() {
    checksum_valid = fl16.get() == msg.data_checksum;
    switch (msg.header.type) {
    case MessageType::SCAN_AP_GET:
